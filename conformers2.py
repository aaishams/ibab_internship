...
def bond_angle(atom2, atom1, atom3, coordinates):
    ...
    mag21 = math.sqrt(vec21[0]**2 + vec21[1]**2 + vec21[2]**2)
    mag23 = math.sqrt(vec23[0]**2 + vec23[1]**2 + vec23[2]**2)
    cos_angle = dot_product / (mag21 * mag23)
...
while answer == 1:
    ...
    if property == 1: #bond length
        ...
        if atom1 > len(coordinates) or atom2 > len(coordinates):
            print(f"ERROR IN INPUT! Atom {atom1} or atom {atom2} or both not in the conformer. PLEASE TRY AGAIN..")
        elif check_length_connectivity(str(atom1), str(atom2), connectivity):
            bl = bond_length(atom1, atom2, coordinates)
            print(f"The bond length between {coordinates[atom1 - 1][0]}[{atom1}] and {coordinates[atom2 - 1][0]}[{atom2}] is {bl:.5f} Å.")
        else:
        ...
        if atom1 > len(coordinates) or atom2 > len(coordinates) or atom3 > len(coordinates):
            print(f"ERROR IN INPUT! Atom {atom1} or atom {atom2} or atom {atom3} or a few of them not in the conformer. PLEASE TRY AGAIN..")
        elif check_angle_connectivity(str(atom2), str(atom1), str(atom3), connectivity):
            ba = bond_angle(atom2, atom1, atom3, coordinates)
            print(f"The bond angle formed by {coordinates[atom1 - 1][0]}[{atom1}], {coordinates[atom2 - 1][0]}[{atom2}] and {coordinates[atom3 - 1][0]}[{atom3}] is {ba:.5f} °.")
        else:
            print(f"{coordinates[atom1 - 1][0]}[{atom1}] and {coordinates[atom3 - 1][0]}[{atom2}] are not connected to {coordinates[atom2 - 1][0]}[{atom3}].")
        ...
        if atom1 > len(coordinates) or atom2 > len(coordinates) or atom3 > len(coordinates) or atom4 > len(coordinates):
            print(f"ERROR IN INPUT! Atom {atom1} or atom {atom2} or atom {atom3} or atom {atom4} or a few of them not in the conformer. PLEASE TRY AGAIN..")
        else:
            da = dihedral_angle(atom1, atom2, atom3, atom4, coordinates)
            print(f"The dihedral angle formed by {coordinates[atom1 - 1][0]}[{atom1}], {coordinates[atom2 - 1][0]}[{atom2}], {coordinates[atom3 - 1][0]}[{atom3}] and {coordinates[atom4 - 1][0]}[{atom4}] is {da:.5f} °.")
        ...
        if not os.path.exists(file_path2):
            ...
            file_path2 = input("Enter the file path: ").strip()
        coordinates2 = extract_coordinates(file_path)
        print(f"COORDINATES OF THE CONFORMER: {coordinates}")
        convert_to_xyz(file_path, coordinates)
        pdb_file2 = file_path.replace(".log", ".pdb")
        connectivity2 = extract_connectivity(pdb_file)
        print(f"CONNECTIVITY DATA OF THE CONFORMER: {connectivity}")
        ...
        if property2 == 1: #compare bond length
            atom1, atom2 = int(input("Enter the first atom: ")), int(input("Enter the second atom: "))
            if atom1 > len(coordinates2) or atom2 > len(coordinates2):
                print(f"ERROR IN INPUT! Atom {atom1} or atom {atom2} or both not in the conformer. PLEASE TRY AGAIN..")
            elif check_length_connectivity(str(atom1), str(atom2), connectivity2):
                bl2 = bond_length(atom1, atom2, coordinates2)
                print(f"The bond length between {coordinates2[atom1 - 1][0]}[{atom1}] and {coordinates2[atom2 - 1][0]}[{atom2}] is {bl2:.5f} Å.")
            else:
                print(f"{coordinates2[atom1 - 1][0]}[{atom1}] and {coordinates2[atom2 - 2][0]}[{atom2}] are not connected.")
        elif property2 == 2: #compare bond angle
            atom1, atom2, atom3 = int(input("Enter the first atom: ")), int(input("Enter the second atom: ")), int(input("Enter the third atom: "))
            if atom1 > len(coordinates2) or atom2 > len(coordinates2) or atom3 > len(coordinates2):
                print(f"ERROR IN INPUT! Atom {atom1} or atom {atom2} or atom {atom3} or a few of them not in the conformer. PLEASE TRY AGAIN..")
            elif check_angle_connectivity(str(atom2), str(atom1), str(atom3), connectivity2):
                ba2 = bond_angle(atom2, atom1, atom3, coordinates2)
                print(f"The bond angle formed by {coordinates2[atom1 - 1][0]}[{atom1}], {coordinates2[atom2 - 1][0]}[{atom2}] and {coordinates2[atom3 - 1][0]}[{atom3}] is {ba2:.5f} °.")
            else:
                print(f"{coordinates[atom1 - 1][0]}[{atom1}] and {coordinates[atom3 - 1][0]}[{atom2}] are not connected to {coordinates[atom2 - 1][0]}[{atom3}].")
        elif property2 == 3: #compare dihedral angle
            atom1, atom2, atom3, atom4 = int(input("Enter the first atom: ")), int(input("Enter the second atom: ")), int(input("Enter the third atom: ")), int(input("Enter the fourth atom: "))
            if atom1 > len(coordinates2) or atom2 > len(coordinates2) or atom3 > len(coordinates2) or atom4 > len(coordinates2):
                print(f"ERROR IN INPUT! Atom {atom1} or atom {atom2} or atom {atom3} or atom {atom4} or a few of them not in the conformer. PLEASE TRY AGAIN..")
            else:
                da2 = dihedral_angle(atom1, atom2, atom3, atom4, coordinates2)
                print(f"The dihedral angle formed by {coordinates2[atom1 - 1][0]}[{atom1}], {coordinates2[atom2 - 1][0]}[{atom2}], {coordinates2[atom3 - 1][0]}[{atom3}] and {coordinates2[atom4 - 1][0]}[{atom4}] is {da2:.5f} °.")
        else:
